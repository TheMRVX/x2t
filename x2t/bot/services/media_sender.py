...
async def send_post_media(
    ...
    """Send extracted Twitter media to Telegram and automatically clean up disk files."""
    if not result.items:
        return []
    ...
    try:
        ...
        # Case 2: Multi-Media (2 to 4 items -> Send as MediaGroup album)
        else:
            media_group = []
            for idx, item in enumerate(result.items):
                file = FSInputFile(item.local_path)
                # First media item gets the full caption
                ...
                else:  # Video or GIF in media group
                    media_group.append(
                        InputMediaVideo(
                            media=file,
                            width=item.width,
            ...
            # Send inline button message below album
        ...
        # Guarantee disk cleanup after sending
