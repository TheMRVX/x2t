"""X / Twitter link downloader message handler."""
...
async def handle_x_link(message: Message, db: Database):
    ...
    try:
        ...
        temp_dir = bot_config.temp_download_dir / f"user_{message.from_user.id}_{tweet_id}"
        result = await x2t.download_media_async(url, output_dir=temp_dir)

        if not result.has_media:
            await status_msg.edit_text(
                "⚠️ <b>این توییت فاقد هرگونه فایل مدیا (ویدیو، گیف یا عکس) است.</b>",
                parse_mode="HTML",
            )
            return
        ...
        logger.error(f"Error processing tweet {url}: {e}", exc_info=True)
        error_text = (
            "❌ <b>خطا در دریافت مدیا!</b>\n\n"
            "ممکن است پست حذف شده باشد، حساب کاربری خصوصی (Private) باشد، یا مشکلی در اتصال پیش آمده باشد.\n\n"
            f"<i>توضیحات خطا:</i> <code>{str(e)[:150]}</code>"
        )
